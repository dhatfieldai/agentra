      ...
      <div className="max-w-6xl mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-8 text-sm">
        ...
        <div className="space-y-4 md:pr-8">
          ...
          <div className="flex flex-col space-y-2">
            <Link
              href="/about"
              className="inline-block text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 font-medium hover:underline transition-colors"
            >
              Read More About Us →
            </Link>
            <Link
...
function FooterLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      className="block hover:underline hover:text-indigo-600 transition-colors md:text-right"
    >
      {children}
    </Link>
  );
